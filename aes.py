...
def sub_bytes(block):
    return bytes([s_box[b >> 4][b & 0x0F] for b in block])

def inv_sub_bytes(block):
    return bytes([inv_s_box[b] for b in block])

def shift_rows(block):
    matrix = [list(block[i::4]) for i in range(4)]
    for i in range(4):
...
text = b"AES demo with your official S-box!!"
